...
return new class extends Migration
{
    ...
    {
        $usersSelect = DB::connection()->getDriverName() === 'mysql'
            ? "GROUP_CONCAT(CONCAT(id, ' <', email, '>') ORDER BY email SEPARATOR ', ') as users"
            : "STRING_AGG(id::text || ' <' || email || '>', ', ' ORDER BY email) as users";

        $duplicateEmails = DB::table('users')
            ->selectRaw('LOWER(email) as normalized_email')
            ->selectRaw('COUNT(*) as user_count')
            ->selectRaw($usersSelect)
