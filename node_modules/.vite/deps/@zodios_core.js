import {
  zod_default
} from "./chunk-O2WMGG5S.js";
import {
  AxiosError,
  axios_default
} from "./chunk-7VM7RDSA.js";
...
var H = { name: "form-data", request: async (o, e) => {
...
var F = { name: "form-url", request: async (o, e) => {
...
function w({ validate: o, transform: e, sendDefaults: t }) {
  return { name: "zod-validation", request: I(o) ? async (i, n) => {
    let s = P(i, n.method, n.url);
    if (!s) throw new Error(`No endpoint found for ${n.method} ${n.url}`);
    let { parameters: d } = s;
    if (!d) return n;
    let p = { ...n, queries: { ...n.queries }, headers: { ...n.headers }, params: { ...n.params } }, f = { Query: (r) => {
      var a;
    ...
    return p;
  } : void 0, response: S(o) ? async (i, n, s) => {
    var p, f, c, m;
...
var B = class {
  constructor(e, t, i) {
    ...
    l(this.api), this.options = { validate: true, transform: true, sendDefaults: false, ...n }, this.options.axiosInstance ? this.axiosInstance = this.options.axiosInstance : this.axiosInstance = axios_default.create({ ...this.options.axiosConfig }), s && (this.axiosInstance.defaults.baseURL = s), this.injectAliasEndpoints(), this.initPlugins(), [true, "all", "request", "response"].includes(this.options.validate) && this.use(w(this.options));
  }
  initPlugins() {
  ...
  injectAliasEndpoints() {
    this.api.forEach((e) => {
      e.alias && (["post", "put", "patch", "delete"].includes(e.method) ? this[e.alias] = (t, i) => this.request({ ...i, method: e.method, url: e.path, data: t }) : this[e.alias] = (t) => this.request({ ...t, method: e.method, url: e.path }));
    });
  }
  async request(e) {
...
var te = B;
function K(o, e) {
  if (o instanceof AxiosError || o && typeof o == "object" && "isAxiosError" in o) {
    let t = o;
    if (t.response) {
      let i = e(t);
...
export {
  ...
  T as formURLPlugin,
  Z as headerPlugin,
  ne as isErrorFromAlias,
  ie as isErrorFromPath,
  z as makeApi,
  W as makeCrudApi,
  G as makeEndpoint,
  V as makeErrors,
  L as makeParameters,
  Y as mergeApis,
  _ as parametersBuilder,
  N as prefixApi,
  w as zodValidationPlugin
};
//# sourceMappingURL=@zodios_core.js.map
