import { createMachine, assign, forwardTo, interpret } from 'xstate';
import { r as makeArrayOf, a3 as wait } from '../../dist/misc-811c3842.esm.js';

const makePoolMachine = params => createMachine({
  id: "pool",
  initial: "idle",
  tsTypes: {},
  schema: {
...
if (import.meta.vitest) {
  ...
  describe("poolMachine", () => {
    ...
    it("should create an async pool that will automatically resolve even if some task failed", async () => {
      const taskList = makeArrayOf(5).map((_, i) => () => i === 1 || i === 3 ? Promise.reject("boom-" + i) : Promise.resolve(i + 1));
...
if (import.meta.vitest) {
  ...
  describe("makeRetryMachine", () => {
    it("should resolve value", () => new Promise(done => {
      ...
      interpret(machine).onTransition((state, event) => {
...
export { createAsyncPool, createPool, makePoolMachine, retryIt };
